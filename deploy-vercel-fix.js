...
function backupCurrentConfig() {
  ...
  if (fs.existsSync('./vercel.json')) {
    fs.copyFileSync('./vercel.json', `${backupDir}/vercel-${timestamp}.json`);
  }
  
  console.log(`✅ Configuration sauvegardée dans ${backupDir}/`);
}
...
function validateVercelConfig() {
  ...
  const checks = [
    ...
    },
    {
      ...
      check: () => vercelConfig.functions && vercelConfig.functions['api/*.js'],
      message: 'Configuration des fonctions API requise'
    }
  ];
...
function buildProject() {
  ...
  try {
    ...
    if (!fs.existsSync('./client/dist')) {
      throw new Error('Build client échoué - dossier client/dist non trouvé');
    }
...
/**
 * Test de l'API en local avant déploiement
 */
async function testApiLocally() {
  console.log('🧪 Test de l\'API en local...');
  
  try {
    execSync('timeout 10s node test-local-api.js', { stdio: 'pipe' });
    console.log('✅ Tests API réussis');
  } catch (error) {
    console.log('⚠️ Tests API non concluants (timeout ou erreur mineure)');
    // Ne pas échouer le déploiement pour cela
  }
}
